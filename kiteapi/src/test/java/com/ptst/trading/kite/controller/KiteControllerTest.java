...
public class KiteControllerTest {
  ...
  @MockitoBean private KitePositionService positionService;

  @MockitoBean private KiteSessionManager sessionManager;
  ...
  public void testGetPositions() throws KiteException, IOException, Exception {
    ...
    mockMvc.perform(get("/api/kite/positions")).andExpect(status().isOk());
  }
  ...
  public void testGetLoginUrl() throws Exception {
    ...
    mockMvc.perform(get("/api/kite/login")).andExpect(status().isOk());
  }
}
