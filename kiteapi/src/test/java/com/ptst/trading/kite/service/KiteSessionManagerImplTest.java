...
class KiteSessionManagerImplTest {
    ...
    @BeforeEach
    void setUp() {
        sessionManager = new KiteSessionManagerImpl(kiteConnect, "api-secret", "http://localhost:8080/api/kite/callback", null);
    }

    @Test
    void constructor_withValidInitialToken_shouldSetToken() {
        KiteSessionManagerImpl sm = new KiteSessionManagerImpl(kiteConnect, "secret", "url", "initial-token");
    ...
    void getLoginURL_shouldDelegateToKiteConnect() {
        when(kiteConnect.getLoginURL()).thenReturn("https://kite.zerodha.com/connect/login?api_key=test&v=3");
    ...
    void authenticate_withoutToken_shouldThrow() {
        IllegalStateException ex = assertThrows(IllegalStateException.class, () -> sessionManager.authenticate());
        assertEquals("Access token is not set. Please authenticate first.", ex.getMessage());
    }

    @Test
    void handleCallback_shouldExchangeTokenAndReturnAccessToken() throws KiteException, IOException, JSONException {
    ...
    @Test
    void handleCallback_whenKiteException_shouldPropagate() throws KiteException, IOException, JSONException {
        when(kiteConnect.generateSession("bad-token", "api-secret"))
                .thenThrow(new KiteException("Invalid token", 400));

        KiteException ex = assertThrows(KiteException.class, () -> sessionManager.handleCallback("bad-token"));
